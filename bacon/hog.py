...
This is a special verison of the Hog project file which uses the C++ extension
"""

from .dice import six_sided, four_sided, make_test_dice
from .ucb import main, trace, interact

from _bacon import config as _config

GOAL_SCORE = _config.GOAL  # The goal of Hog is to score 100 points.
...
def free_bacon(score):
    ...
    # BEGIN PROBLEM 2
    return _config.free_bacon(score)
    # END PROBLEM 2
...
def is_swap(player_score, opponent_score):
    ...
    """
    return _config.is_swap(player_score, opponent_score)
...
def announce_highest(who, previous_high=0, previous_score=0):
    ...
    def announce(score0, score1):
        if (who == 0): 
           ...
           cur_score = score1
        diff = cur_score - previous_score
        if (diff > previous_high): 
            print (diff, "point(s)! That\'s the biggest gain yet for Player", who)
        return announce_highest (who, max (diff, previous_high), cur_score)
    return announce
    # END PROBLEM 7
...
def swap_strategy(score, opponent_score, margin=8, num_rolls=4):
    ...
    if ((potential_score-score >= margin) and (opponent_score >= potential_score or not (is_swap(potential_score, opponent_score)))): 
        return 0
    elif (opponent_score > potential_score and is_swap(potential_score, opponent_score)): 
        return 0
    else: 
        return num_rolls
    # END PROBLEM 11
...
# hack, remove later
def roll_at_least(goal, i):
    return -(i-4)**2 * 1.25 + 10.0
